...
export default function ExpenseType({ netIncome = 35440 }: ExpenseTypeProps) {
	...
	const totals =
		exp_data?.reduce((acc: Record<string, number>, curr) => {
						...
						<>
							<div key={type} className="flex-1 px-4">
								<div className="text-center">
									...
									<p className={`text-sm font-medium mb-1 ${status.textColor}`}>
							...
							{index < expenseEntries.length - 1 && (
								<div className="w-px bg-gray-200 dark:bg-gray-700 mx-2 my-4" />
							)}
						</>
					);
				})}
