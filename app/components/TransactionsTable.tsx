...
export const columns: ColumnDef<Transactions>[] = [
	{
		...
		cell: ({ row }) => {
			...
			return (
				<div className="text-blue-500 font-medium">{`${day}${getOrdinalSuffix(day)} ${month} ${year}`}</div>
			);
		},
	},
	{
		...
		cell: ({ row }) => {
			...
			return <div className=" font-inconsolata">{amount}</div>;
		},
	},
	{
		...
		cell: ({ row }) => {
			...
			return (
				<div className="text-muted-foreground font-inconsolata">{cost}</div>
			);
		},
	},
	{
		...
		cell: ({ row }) => {
			...
			return (
				<span
					className={`px-2 py-1 rounded-md text-xs font-medium ${getCategoryStyle(category)}`}
				>
					{category}
				</span>
			);
							...
							<TableRow key={headerGroup.id} className="border-b">
								{headerGroup.headers.map((header) => {
									return (
										<TableHead
											key={header.id}
											className="border-r last:border-r-0 py-2 px-4"
										>
						...
						{table.getRowModel().rows?.length ? (
							...
							<TableRow className="border-b">
								<TableCell
									...
									className="h-12 text-center border-r last:border-r-0 py-2 px-4"
								>
									No results.
								</TableCell>
							</TableRow>
